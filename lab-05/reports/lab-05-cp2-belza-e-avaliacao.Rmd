...
```
Com base ba Tabela acima, é possível mais professores do sexo masculino em comparação ao sexo feminino.

Ainda no contexto de entender melhor os dados, as Tabelas abaixo foram produzidas. Nelas, estão presentes as quantidades de para as variáveis:

* ***ethnicity:*** Etnia do professo, variando entre minority (minoria) e not minority (não minoria);
* ***pic_outfit:*** Roupa do professor na foto utilizada na avaliação variando entre formal (formal) e not formal (não formal);
* ***pic_color:*** cor da foto avaliada colorida (color) ou preto e branco (black&white);
* ***rank:*** Nível do professor, variando entre horista (teaching), assitente (tenure track) e titular (tenured);
...
##2. Relação entre score dos professores e sua beleza

Inicialmente foi verificado se existia uma relação entre a pontuação (score) e a beleza (bty_avg) dos professores. Deste modo, A Figura 6 foi produzida.
...
avaliacao_professores %>% 
  ...
  labs(title="Figura 6 - Relação entre Score e Beleza dos professores")
```
```{r}
modelo1 <- lm(score ~ bty_avg, data = avaliacao_professores)
